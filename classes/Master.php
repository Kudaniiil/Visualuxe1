...
Class Master extends DBConnection {
	...
	function save_album(){
		...
		if($check > 0){
			...
			if($save){
				$resp['status'] = "success";
				$this->settings->set_flashdata('success'," Album Berhasil Disimpan");
			}else{
				$resp['status'] = "failed";
				$resp['sql'] = $sql;
			}
	...
	function delete_album(){
		...
		if($delete){
			...
			$this->settings->set_flashdata('success'," Album Berhasil di Hapus. Data akan disimpan di arsip sementara");
		}else{
			$resp['status'] = "failed";
			$resp['sql'] = $sql;
		}
	...
	function retrieve_album(){
		...
		if($retrieve){
			...
			if($retrieve_imgs){
				$this->settings->set_flashdata('success'," ALbum Berhasil diPulihkan");
				$resp['status'] = 'success';
			}else{
				$resp['status'] = 'failed';
	...
	function save_image(){
		...
		if(isset($_FILES['images']['tmp_name']) && count($_FILES['images']['tmp_name'])>0){
			...
			if(!empty($data)){
				...
				if($data){
					$resp['status'] = 'success';
					$this->settings->set_flashdata('success',' Gambar Berhasil Diunggah');
				}else{
					$resp['status'] = 'failded';
					$resp['error'] = $this->conn->error;
					$resp['sql'] = $sql;
	...
	function rename_image(){
		...
		if($save){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success',' Gambar Berhasil Di Ganti Nama');
		}else{
			$this->settings->set_flashdata('success',' Gambar Berhasil Di Ganti Nama');
			$resp['status'] = 'failed';
			$resp['msg'] = "Error Renaming images.";
			$resp['error'] = $this->conn->error;
			$resp['sql'] = $sql;
		}
		return json_encode($resp);
	...
	function delete_image(){
		...
		if($delete){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success'," Gambar Berhasil Dihapus. Data Akan Disimpan di Arsip Sementara");
		}else{
			$resp['status'] = "failed";
			$resp['error'] = $this->conn->error;
			$resp['sql'] = $sql;
	...
	function move_image(){
		...
		if($move){
			...
			if($save){
				$resp['status'] = 'success';
				$this->settings->set_flashdata('success'," Gambar Berhasil Di Pindahkan");
			}else{
				$resp['status'] = "failed";
				$resp['error'] = $this->conn->error;
	...
	function retrieve_image(){
		...
		if($retrieve){
			...
			$retrieve_imgs = $this->conn->query($sql);
			if($retrieve_imgs){
				$this->settings->set_flashdata('success'," Gambar Berhasil Dipulihkan");
				$resp['status'] = 'success';
			}else{
				$resp['status'] = 'failed';
				$resp['msg'] = 'Gambar Berhasil Dipulihkan, Album Gagal';
				$resp['error'] = $this->conn->error;
				$resp['sql'] = $sql;
			}
		}else{
	...
	function permanently_delete(){
		...
		$this->settings->set_flashdata('success'," Semua Arsip Berhasil Dihapus");
		$resp['status'] = 'success';
		return json_encode($resp);
	}
